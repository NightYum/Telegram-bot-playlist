...
async def cmd_create_playlist_next(msg: types.Message, state: FSMContext):
    ...
    if len(user_text) > 25:
        await msg.answer(text="Максимальная длина названия 25, введите данные снова")
        await state.finish()
    else:
...
async def cmd_test_hand(msg: types.Message, state: FSMContext):
    await state.update_data(count_video=int(msg.text))
...
async def cmd_test_hand(msg: types.Message, state: FSMContext):
    await state.update_data(count_viewed=int(msg.text))
...
async def cmd_test_hand(msg: types.Message, state: FSMContext):
    await state.update_data(count_vid_in_day=msg.text)
...
async def cmd_delete_playlist_hand(msg: types.Message, state: FSMContext):
    ...
    await msg.answer(text="Удаление завершено удачно, чтобы это проверить - /view")
    await state.update_data(delete_playlist=playlist_id)
    await state.finish()
...
async def cmd_view_detail_static_hand(msg: types.Message, state: FSMContext):
    ...
    await state.update_data(choose_playlist=user_id)
    await state.finish()
...
# Запись файлов пользоватлея, статистика, рейнтинг получение по сслыке сколько осталось в процентах
